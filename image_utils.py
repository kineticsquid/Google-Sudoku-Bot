...
def extract_matrix_from_image(matrix_image, tesseract_config=None, image_type=None, flask_app=None):
    ...
    log('Start Time: %.2f' % start_time, flask_app)
    ...
    log('End Time: %.2f' % (time.time() - start_time), flask_app)
...
def get_image_type(image, flask_app=None):
    ...
    log('Image type: %s' % image_type, flask_app)
    return image_type
...
def get_tesseract_config_based_on_image_type(image_type, flask_app=None):
    ...
    log('Tesseract config: %s' % tesseract_config, flask_app=flask_app)
...
def process_image(image, blur, threshold, x_s, y_s, input_matrix, tesseract_config, done_event, flask_app=None):
    ...
    start = time.time()
    log("Starting %s - blur: %s, threshold: %s" % (threading.get_ident(), blur, threshold), flask_app)
    # blur the image
...
def get_cell_boundaries(image, flask_app=None):
    ...
    def refactor_coords(coords, coord_deltas):
        ...
        log('Coordinate widths histogram for refactoring:', flask_app)
        log(widths_histogram, flask_app)
        ...
        min_width = min_width * .85
        max_width = max_width * 1.1
        log('Min width: %s, Max width: %s' % (min_width, max_width), flask_app)
        ...
        # start of cell coordinates i and the end of cell coordinates i-1 that is a multiple of the identified cell 
        ...
        if len(new_coords) < 9:
            ...
            while True:
                ...
                elif min_width <= (gap - 3 * average_line_width) / 3 <= max_width:
                    # found a gap of width of three cells
                    cell_width = int((gap - average_line_width * 4) / 3)
                    new_entry_1 = [new_coords[i - 1][1] + average_line_width,
                                   new_coords[i - 1][1] + average_line_width + cell_width]
                    new_entry_2 = [new_coords[i - 1][1] + cell_width + average_line_width * 2,
                                   new_coords[i - 1][1] + cell_width * 2 + average_line_width * 3]
        ...
        if len(new_coords) < 9:
            ...
            while True:
                ...
                if 2 * min_width + average_line_width <= gap <= 2 * max_width + average_line_width:
                    # found a set of coordinates which appear to be two cells
                    new_entry_1 = [new_coords[i][0], new_coords[i][0] + int(gap/2) - average_line_width]
                    new_entry_2 = [new_coords[i][1] - int(gap/2) + average_line_width, new_coords[i][1]]
                    new_coords[i] = new_entry_2
                    new_coords.insert(i, new_entry_1)
                elif 3 * min_width + average_line_width <= gap <= 3 * max_width + 2 * average_line_width:
                    # found a set of coordinates which appear to be three cells
                    new_entry_1 = [new_coords[i][0], new_coords[i][0] + int(gap/3) - average_line_width]
                    new_entry_2 = [new_coords[i][0] + int(gap/3) + average_line_width, new_coords[i][1] - int(gap/3) - average_line_width]
                    new_entry_3 = [new_coords[i][1] - int(gap/3) + average_line_width, new_coords[i][1]]
                    new_coords[i] = new_entry_3
                    new_coords.insert(i, new_entry_2)
                    new_coords.insert(i, new_entry_1)
        ...
        if len(new_coords) > 9:
            ...
            while len(new_coords) > 9:
                if abs(abs(new_coords[0][0] - new_coords[0][1]) - new_coord_delta_avg) > abs(abs(new_coords[len(new_coords)-1][0] - new_coords[len(new_coords)-1][1]) - new_coord_delta_avg):
                    log("Removing extra coord 0.", flask_app)
                    new_coords.pop(0)

                else:
                    log("Removing extra coord %s." % (len(new_coords) - 1), flask_app)
                    new_coords.pop(len(new_coords)-1)

        return new_coords

    log('x coords before refactoring: %s' % x_coords, flask_app)
...
def find_lines(image, flask_app=None):
    ...
    log('Image height: %s' % image_height, flask_app)
    log('Minimum side: %s' % minimum_side, flask_app)
    log('Minimum line length: %s' % min_line_length, flask_app)
    log('Max line gap: %s' % max_line_gap, flask_app)
    ...
    log("Horizontal lines: %s" % len(horizontal_lines), flask_app)
    log("Vertical lines: %s" % len(vertical_lines), flask_app)
    log("Rejected lines: %s" % len(rejected_lines), flask_app)
